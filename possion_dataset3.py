...
def load_background(width, height):
    ...
    for file in files:
        ...
        backgrounds.append(cp.asarray(cv2.resize(img, (width, height))))


def create_mask(width, height, boxes):
    img = np.zeros((width, height, 3), dtype=cp.uint8)
    for box in boxes:
...
def source_elements(source_name):
    ...
    mask = cp.asarray(cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY))
    M = cp.float32([[1, 0, 0], [0, 1, 0]])
    ...
    source = cp.asarray(cv2.warpAffine(source, M, (x_range, y_range)))

    mask = mask[y_min:y_max, x_min:x_max]
    mask[mask != 0] = 1

    mat_A = laplacian_matrix(y_range, x_range)
    laplacian = cupyx.scipy.sparse.csr_matrix(mat_A, copy=True)
    for y in range(1, y_range - 1):
        for x in range(1, x_range - 1):
            if mask[y, x] == 0:
                ...
                mat_A[k, k + x_range] = 0
                mat_A[k, k - x_range] = 0
    mat_A = cupyx.scipy.sparse.csr_matrix(mat_A)
...
def edit(source_name, source, mat_A, mask_flat, laplacian, target):
    ...
    name = uuid.uuid4()
    cv2.imwrite(path.join(INPUT_DIR, "images", str(name) + ".jpg"), cp.asnumpy(target))
